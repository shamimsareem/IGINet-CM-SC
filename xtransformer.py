...
#from layers.tensor_sketch1 import TensorSketchPooling
...
class XTransformer(BasicModel):
    def __init__(self):
        ...
        input_dim1 = cfg.MODEL.ATT_FEATS_DIM
        input_dim2 = cfg.MODEL.ATT_FEATS_DIM
        output_dim = cfg.MODEL.BILINEAR.DIM  # Assuming the output dimension is the same as BILINEAR.DIM
        # att_feats encoder
        sequential = []
       ...
       # self.cbp = TensorSketchPooling(input_dim1, input_dim2, output_dim)
    ...
    def forward(self, **kwargs):
        ...
        #encoder_out = self.cbp(encoder_out, encoder_out)

        #print(encoder_out.size())
        # Repeat each element 768/60 times along the second dimension
        #encoder_out = encoder_out.repeat(1, 13)
        #encoder_out = encoder_out[:, :768]
        #print(encoder_out.size())
    ...
    def decode(self, **kwargs):
        ...
        batch_size = att_feats.size(0)
        att_feats = self.att_embed(att_feats)
        att_feats = self.cbp(att_feats, att_feats)
        gx, encoder_out = self.encoder(att_feats, att_mask)
        p_att_feats = self.decoder.precompute(encoder_out)
        self.decoder.init_buffer(batch_size)
