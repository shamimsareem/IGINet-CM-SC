...
class AttBasicModel(BasicModel):
    def __init__(self):
        ...
        # geometric feat embed
        sequential = []
        if cfg.MODEL.ATT_FEATS_EMBED_DIM > 0:
            sequential.append(nn.Linear(4, cfg.MODEL.ATT_FEATS_EMBED_DIM))
        ...
        self.gtt_feat_embed = nn.Sequential(*sequential) if len(sequential) > 0 else None
    ...
    def make_kwargs(self, wt, gv_feat, att_feats, gtt_feats, att_mask, p_att_feats, state, **kgs):
        kwargs = kgs
        kwargs[cfg.PARAM.WT] = wt
        kwargs[cfg.PARAM.GLOBAL_FEAT] = gv_feat
        kwargs[cfg.PARAM.ATT_FEATS] = att_feats
        kwargs[cfg.PARAM.GTT_FEATS] = gtt_feats
    ...
    def preprocess(self, **kwargs):
        ...
        #print("geometric shape allignment:")
        #print(att_feats.shape)
        #exit(0)

        if self.gtt_feat_embed is not None:
        ...
        # bilinear
        if cfg.MODEL.BILINEAR.DIM > 0:
            gv_feat, att_feats, gtt_feats = self.encoder_layers(gv_feat, att_feats, gtt_feats, att_mask)
    ...
    def forward(self, **kwargs):
        ...
        gv_feat, att_feats, gtt_feats, att_mask, p_att_feats = self.preprocess(**kwargs)
        ...
        for t in range(seq.size(1)):
            if self.training and t >=1 and self.ss_prob > 0:
                ...
                if mask.sum() == 0:
                    ...
                    wt = wt.long()
                    wt.index_copy_(0, ind, torch.multinomial(prob_prev, 1).view(-1).index_select(0, ind).long())
            else:
                wt = seq[:,t].clone()
            ...
            kwargs = self.make_kwargs(wt, gv_feat, att_feats, gtt_feats, att_mask, p_att_feats, state)
    ...
    def decode_beam(self, **kwargs):
        ...
        for t in range(cfg.MODEL.SEQ_LEN):
            ...
            if t == 0:
                ...
                gtt_feats = utils.expand_tensor(gtt_feats, beam_size)
